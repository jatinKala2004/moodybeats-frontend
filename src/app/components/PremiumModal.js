...
export default function PremiumModal({ open, onClose, onUpgrade, isPremium, isLoggedIn, onRequireLogin, onCancel }) {
  if (!open) return null;
  const features = [
          ...
          {!isPremium ? (
            ...
            <>
              ...
              {onCancel && (
                <button
                  onClick={onCancel}
                  className="w-full px-4 py-2 bg-transparent border border-red-500 text-red-400 font-semibold rounded-lg hover:bg-red-500/10 transition-colors text-sm mb-1"
                >
                  Cancel subscription
                </button>
              )}
